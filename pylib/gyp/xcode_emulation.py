...
class XcodeSettings(object):
  ...
  def GetWrapperExtension(self):
    ...
    if self.spec['type'] in ('loadable_module', 'shared_library'):
      ...
      assert (
          False
      ), f"Don't know extension for '{self.spec['type']}', target '{self.spec['target_name']}'"
  ...
  def _MapLinkerFlagFilename(self, ldflag, gyp_to_build_path):
    ...
    for flag_pattern in linker_flags:
      ...
      if m := regex.match(ldflag):
        ldflag = ldflag[:m.start(1)] + gyp_to_build_path(m[1]) + ldflag[m.end(1):]
  ...
  def GetLdflags(self, configname, product_dir, gyp_to_build_path, arch=None):
    ...
    if sdk_root and is_extension:
      ...
      if XcodeVersion() < '0900':
        ldflags.extend((
            '-lpkstart',
            f'{sdk_root}/System/Library/PrivateFrameworks/PlugInKit.framework/PlugInKit',
        ))
      else:
  ...
  def GetPerTargetSetting(self, setting, default=None):
    ...
    for configname in sorted(self.xcode_settings.keys()):
      if is_first_pass:
        ...
        assert result == self.xcode_settings[configname].get(
            setting, None
        ), f"Expected per-target setting for '{setting}', got per-config setting (target {self.spec['target_name']})"
...
def IsMacBundle(flavor, spec):
  ...
  if is_mac_bundle:
    assert (
        spec['type'] != 'none'
    ), f"""mac_bundle targets cannot have type none (target "{spec['target_name']}")"""
...
def _AddIOSDeviceConfigurations(targets):
  ...
  for target_dict in targets.itervalues():
    ...
    for config_name, simulator_config_dict in dict(configs).iteritems():
      ...
      configs[f'{config_name}-iphonesimulator'] = simulator_config_dict
      if toolset == 'target':
