...
def _TargetFromSpec(old_spec, params):
  ...
  if ninja_toplevel:
    new_xcode_settings[
        'CONFIGURATION_BUILD_DIR'] = f"{ninja_toplevel}/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)"
  ...
  if ninja_toplevel:
    ninja_target['actions'] = [{
        'action_name':
        f'Compile and copy {target_name} via ninja',
        'inputs': [],
        'outputs': [],
        'action': [
            'env',
...
def IsValidTargetForWrapper(target_extras, executable_target_pattern, spec):
  ...
  if (int(spec.get('mac_xctest_bundle', 0)) != 0 or
    ...
    return bool(executable_target_pattern is None
...
def CreateWrapper(target_list, target_dicts, data, params):
  ...
  if main_gyp is None:
    ...
    main_gyp = f"{build_file_root}.ninja{build_file_ext}"
    ...
    if IsValidTargetForWrapper(target_extras, executable_target_pattern, spec):
      ...
      for old_target in data[old_qualified_target.split(':')[0]]['targets']:
        if old_target['target_name'] == target_name:
          new_data_target = {
              'target_name': old_target['target_name'],
              'toolset': old_target['toolset'],
          }
  ...
  sources_gyp = os.path.join(os.path.dirname(main_gyp),
                             f"{sources_target_name}.gyp")
  ...
  new_data_target = {
      'target_name': sources_target['target_name'],
      '_DEPTH': depth,
      'toolset': "target",
  }
  new_data[sources_gyp] = {'targets': [], 'included_files': []}
