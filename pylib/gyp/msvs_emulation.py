...
def ExtractSharedMSVSSystemIncludes(configs, generator_flags):
  ...
  for config in configs:
    if includes := config.get('msvs_system_include_dirs', []):
...
class MsvsSettings(object):
  ...
  def __init__(self, spec, generator_flags):
    ...
    for field in unsupported_fields:
      for config in configs.values():
        if field in config:
          unsupported += [f"{field} not supported (target {spec['target_name']})."]
    if unsupported:
      raise Exception('\n'.join(unsupported))
  ...
  def GetExtension(self):
    ...
    the target type.
    """
    if ext := self.spec.get('product_extension', None):
      return ext
  ...
  def GetVSMacroEnv(self, base_to_build=None, config=None):
    ...
    target_ext = f'.{self.GetExtension()}'
    ...
    replacements |= GetGlobalVSMacroEnv(self.vs_version)
    return replacements
  ...
  def AdjustLibraries(self, libraries):
    ...
    return [
        f'{lib}.lib' if not lib.lower().endswith('.lib') else lib for lib in libs
    ]
  ...
  def _GetPchFlags(self, config, extension):
    ...
    if self.msvs_precompiled_header[config]:
      ...
      if _LanguageMatchesForPch(source_ext, extension):
        ...
        return [f'/Yu{pch}', f'/FI{pch}', '/Fp${pchprefix}.' + pchbase + '.pch']
    return  []
  ...
  def GetLdflags(self, config, gyp_to_build_path, expand_special,
    ...
    ld(
        'SubSystem',
        map={
            '1': f'CONSOLE{minimum_required_version}',
            '2': f'WINDOWS{minimum_required_version}',
        },
        prefix='/SUBSYSTEM:',
    )

    if stack_reserve_size := self._Setting(('VCLinkerTool', 'StackReserveSize'),
                                           config,
                                           default=''):
      stack_commit_size = self._Setting(
          ('VCLinkerTool', 'StackCommitSize'), config, default='')
      if stack_commit_size:
        stack_commit_size = f',{stack_commit_size}'
  ...
  def _GetLdManifestFlags(self, config, name, gyp_to_build_path,
    ...
    flags = ['/MANIFEST', f'/ManifestFile:{output_name}', '/MANIFESTUAC:NO']
...
<trustInfo xmlns="urn:schemas-microsoft-com:asm.v3">
  <security>
    ...
    generated_name = f'{name}.generated.manifest'
...
def GenerateEnvironmentFiles(toplevel_build_dir, generator_flags,
  ...
  if generator_flags.get('ninja_use_custom_environment_files', 0):
    return {arch: 'cl.exe' for arch in archs}
  vs = GetVSVersion(generator_flags)
  cl_paths = {}
  for arch in archs:
